...
# Here we are using namecorr we defined above the split 'Date' values in 'date' column into month and year values in order to
# be able to add 'month' and 'year' columns to dataset.
...
while degerson < 0.85:
    X_train, X_test, y_train, y_test = train_test_split(train.drop(['item_price', 'date_block_num', 'item_cnt_day'], axis = 1), 
                                                         train['item_cnt_day'], test_size=0.3)
    
    modelf = RandomForestRegressor(n_estimators = 50)
...
while degermson < 0.85:
    ...
    degerm = cross_validate(models, X_test, y_test, cv=4) 
...
while degerbson < 0.85:
    ...
    modelt = RandomForestRegressor(n_estimators = 50) 
    modelt.fit(X_train, y_train) 
    degerb = cross_validate(modelt, X_test, y_test, cv=4) 
